 ...
 watch:{
  ...
  methods:{
	...
	fetchUser(){
		...
		axios.get(SERVER_URL + 'api/clocks/' + this.$route.params.userId).then((response)=>{
  	...
  	onClick(){
		...
		axios.get(SERVER_URL+'api/clocks/'+this.$route.params.userId).then((response)=>{
			...
			if(this.status_user === false){
				...
				axios.put(SERVER_URL+'api/clocks/'+this.$route.params.userId ,body,headers ).then((response) => {
						this.$refs.ref_clock.startTimer() ; 
						button.style.backgroundColor = 'green' ;
				} , (error)=>{ console.log(error) ;})
				...
				var headers = {'Content-Type' : 'application/json'} ; 
				axios.put(SERVER_URL+'api/clocks/'+this.$route.params.userId ,body,headers ).then((response) => {							this.$refs.ref_clock.stopTimer(); 
						button.style.backgroundColor = 'red' ;	
			...
			this.$router.push({name :'clock' , params:{userId:this.$route.params.userId} }).catch(error => {
					if(error.name != 'NavigationDuplicated') 
						throw error ; 
			});
		} , (error) => {
			console.log('Get clocks error') ; }
		); 
