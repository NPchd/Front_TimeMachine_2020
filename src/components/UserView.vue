<template>
  <div class="userView">
	...
	<form id="adduser">
		<p> username :<input type="text" v-model="username" ></br>
...
export default {
  ...
  methods:{
	...
	createUser(args){
		...
		} ,(error)=>{this.response = error ; this.error = 'ko' ; console.log(error)}); 
	},
	...
	getUser(struct){
		var URL = SERVER_URL ; 
		axios.get(URL+"api/users?email="+struct.email+"&username="+struct.username).then((response) => {
			...
			this.$router.push({name:'dashboard' , params:{userId:this.user_id}}).catch(error => {
			...
			console.log(response); 
		},(error)=> {this.response = "User :"+struct.username+" does not exist"  ; this.error = 'ko' ; console.log(error) ;}); 
	...
	deleteUser(args){
		...
		axios.delete(URL).then((response) => {
			...
			console.log(response) ; 
		},(error) => { this.response = "User not found , cannot delete" ;this.error = 'ko' ; console.log(error) ;}) ; 
